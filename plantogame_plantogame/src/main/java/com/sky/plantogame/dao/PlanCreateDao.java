...
public interface PlanCreateDao extends JpaRepository<PlanCreate,String>,JpaSpecificationExecutor<PlanCreate>{
    ...
    @Query(value = "SELECT * FROM tb_plan_create WHERE  type =?1  AND gamekey = ?2 AND create_time BETWEEN DATE(NOW())  AND gamekey = '' ORDER BY create_time DESC limit 0,300 ",nativeQuery = true)
